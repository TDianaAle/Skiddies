<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
    ...
    <div class="flex min-h-screen">
      ...
      <!-- Main Content -->
      <main class="flex-1 transition-all duration-300 ease-in-out justify-center"
        :class="isSidebarOpen ? 'ml-0' : 'ml-20'" role="main" aria-label="Dashboard principale del tutor">
          ...
          <section
            ...
            <div class="flex items-center gap-3 mb-6">
              <div class="p-3 bg-blue-100 rounded-full">
                <img src="\img\upload.png" alt="Upload Icon" class="w-6 h-5.5" />
              </div>
            ...
            <form @submit.prevent="uploadVideo" class="space-y-6">
              <div class="space-y-2">
                ...
                <input id="video-title" v-model="newVideo.title" type="text" placeholder="Inserisci il titolo del video"
              ...
              <div class="space-y-2">
                ...
                <select id="video-category" v-model="newVideo.category" required aria-describedby="category-error"
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  <input id="video-file" type="file" accept="video/*" @change="handleFileChange" required
              ...
              <button type="submit"
                ...
                <span class="flex items-center justify-center gap-2">
                  <img src="/img/upload2.png" alt="Upload Icon" class="w-5 h-4.5" />
          ...
          <section id="video-section" aria-labelledby="videos-heading">
            <div class="flex items-center justify-between mb-6">
              <div class="flex items-center gap-3">
                <div class="p-3 bg-purple-100 rounded-full">
                  <img src="/img/video2.png" alt="Video Icon" class="w-6 h-6 text-purple-600" />
                </div>
            ...
            <div v-if="videos.length === 0"
              ...
              <div class="p-6 bg-gray-100 rounded-full w-24 h-24 mx-auto mb-6 flex items-center justify-center">
                <img src="/img/video.png" alt="Video icon" class="w-8 h-8 " />
              </div>
              ...
              <button @click="scrollToUploadForm"
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6"
              role="grid" aria-label="Griglia dei video caricati">
              <div v-for="video in videos" :key="video.id" role="gridcell" tabindex="0"
                ...
                @click="toggleVideo(video.id)" @keydown.enter="toggleVideo(video.id)"
                @keydown.space.prevent="toggleVideo(video.id)" :aria-label="`Apri video: ${video.title}`">
                <!-- Video Thumbnail -->
                <div class="relative overflow-hidden rounded-t-2xl">
                  <video :src="`http://localhost/skiddies/backend/${video.file_path}`"
                  ...
                  <div
                    class="absolute top-3 right-3 bg-black/50 text-white px-2 py-1 rounded-lg text-xs backdrop-blur-sm">
                  ...
                  <div
                    ...
                    <div
                      class="bg-white/90 rounded-full p-4 transform scale-75 group-hover:scale-100 transition-transform duration-200">
                      <img src="/img/play.png" alt="Video icon" class="w-3.5 h-4 " />
                    </div>
                  </div>
                </div>
                ...
                <div class="p-5 space-y-3">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <div class="flex items-center gap-1 text-red-500 shrink-0" :title="`${video.likes} mi piace`">
                      <img src="/img/heart.png" alt="mi piace" class="w-4 h-4" />
                  ...
                  <div class="flex items-center justify-between pt-2">
                    <button @click.stop="deleteVideo(video.id)"
                      ...
                      :aria-label="`Elimina video: ${video.title}`" title="Elimina video">
                      <img src="/img/trash.png" alt="" class="w-3 h-3" aria-hidden="true" />
                      Elimina
                    </button>


                    <button @click.stop="toggleVideo(video.id)"
                      ...
                      <img src="/img/eye.png" alt="Dettagli" class="w-3.5 h-3.5" />
                      Dettagli
                    </button>
                  ...
                  <div class="border-t pt-3 mt-3">
                    <div v-if="commentsMap[video.id] && commentsMap[video.id].length > 0" class="space-y-2">
                      <div class="flex items-center gap-2 text-sm font-semibold text-gray-700">
                        <img src="/img/message.png" alt="Comment icon" class="w-4 h-4 opacity-40 filter grayscale" />
                        {{ commentsMap[video.id].length }} commento{{ commentsMap[video.id].length !== 1 ? 'i' : '' }}
                      </div>

                      <div class="max-h-20 overflow-y-auto space-y-1">
                        <div v-for="comment in commentsMap[video.id].slice(0, 2)" :key="comment.id"
            ...
            <!-- Video Detail Modal -->
            <div v-if="expandedVideo"
              class="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 p-4" role="dialog"
              aria-modal="true" :aria-labelledby="`modal-title-${expandedVideo}`" @click.self="toggleVideo(null)">
              <div
                class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-y-auto relative animate-pulse">
                <div class="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-100 p-6 rounded-t-2xl">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <h3 :id="`modal-title-${expandedVideo}`" class="text-2xl font-bold text-gray-800 mb-2 pr-8">
                    ...
                    <button @click="toggleVideo(null)"
                      ...
                      <img src="/img/x.png" alt="Chiudi" class="w-4 h-4" />
                    </button>
                ...
                <div class="p-6 space-y-6">
                  ...
                  <div class="relative bg-gray-900 rounded-xl overflow-hidden">
                    <video v-if="getExpandedVideo" :key="getExpandedVideo.id"
                      :src="`http://localhost/skiddies/backend/${getExpandedVideo.file_path}`" controls playsinline
                  ...
                  <div class="bg-gray-50 rounded-xl p-6">
                    ...
                    <div v-if="commentsMap[expandedVideo] && commentsMap[expandedVideo].length > 0"
                      class="space-y-4 max-h-60 overflow-y-auto">
                      <div v-for="comment in commentsMap[expandedVideo]" :key="comment.id"
                    ...
                    <div v-else class="text-center py-8 text-gray-500">
                      <img src="/img/message.png" alt="Messaggio"
                        class="w-8 h-8 mx-auto mb-3 opacity-40 filter grayscale" />
                  ...
                  <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                    <button @click="deleteVideo(getExpandedVideo?.id)"
                      ...
                      <img src="/img/trash2.png" alt="" class="w-5 h-5" aria-hidden="true" />
